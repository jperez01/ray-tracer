...
Color World::shadeHit(Computations &computations, int remaining) {
    ...
    Vector3f normal = computations.normal;
    Shape *shape = computations.object;
    Vector3f viewDir = computations.eye;
    Point3f finalPosition = computations.point;
    Point3f overPoint = computations.over_point;
    ...
    for (PointLight &light : m_pointLights) {
        ...
        finalColor = finalColor + light.calculateColorFromPoint(normal, overPoint, viewDir, *shape, inShadow);
    }
    
    for (DirectionalLight &light : m_directLights) {
        finalColor = finalColor + light.calculateColorFromDirection(normal, viewDir, shape);
    }
    finalColor.clamp();
...
Color World::findRefraction(Computations &info, int remaining) {
    ...
    Vector3f direction = info.normal * (n_ratio * cos_i - cos_t)
        - info.eye * n_ratio;
    
    Ray refract_ray(info.under_point, direction);
...
bool World::isShadowed(Point3f &point, PointLight &light) {
